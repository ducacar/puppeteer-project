...
async function scrapeEtsy() {
    ...
    const browser = await puppeteer_1.default.launch({ headless: true }); //change to true latter
    ...
    try {
        ...
        const products = await page.evaluate(() => {
            const productList = Array.from(document.querySelectorAll(".js-merch-stash-check-listing")).slice(0, 2);
            return productList.map((product) => {
                const nameElement = product.querySelector(".wt-text-caption.streamline-seller-shop-name__line-height");
                let name = "N/A";
                ...
                // Extract product URL
                const urlElement = product.querySelector("a.listing-link");
        ...
        for (const product of products) {
            if (product.url !== "N/A") {
                ...
                const details = await page.evaluate(() => {
                    ...
                    const sizeOptions = Array.from(document.querySelectorAll("#variation-selector-0 option"));
                ...
                async function selectDropdownOption(page, selector) {
                    ...
                    for (const dropdownHandle of dropdownHandles) {
                        ...
                        if (options.length > 1) {
                            const optionValue = await options[1].evaluate((node) => node.value);
                ...
                async function fillTextarea(page, selector, message) {
                    try {
                ...
                try {
                    ...
                    await fillTextarea(page, "#listing-page-personalization-textarea", "My custom message");
                }
                catch (error) {
                    console.error("An error occurred while filling the textarea:", error);
                }
                // Click the "Add to cart" button
                await page.waitForSelector("div[data-add-to-cart-button] button.wt-btn--filled:not([disabled])");
                const addButtonHandle = await page.$("div[data-add-to-cart-button] button.wt-btn--filled:not([disabled])");
                if (addButtonHandle) {
                ...
                console.log("Added to cart and proceeded to checkout for:", product.name);
            }
            // Write the array containing all product information to the JSON file
            fs.writeFile("products.json", JSON.stringify(productsWithDetails, null, 2), (err) => {
                if (err) {
                    console.error("Error writing to file:", err);
                }
                else {
        ...
        await page.waitForSelector('.wt-btn.wt-btn--secondary.wt-width-full[data-join-neu-button=""]');
        await page.click('.wt-btn.wt-btn--secondary.wt-width-full[data-join-neu-button=""]');
        ...
        await page.click('.wt-btn.wt-btn--filled.wt-width-full[data-selector-save-btn=""]');
